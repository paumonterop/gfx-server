...
const Operation_OnTime = () => {
    ...
    const showClasButton = (bool) => {
      ...
      socket.on("connect", () => {
        ...
        if (bool) {
          socket.emit("showOnTimeOn", true); // Emitir los datos formateados
        } else {
          socket.emit("showOnTimeOff", false); // Emitir los datos formateados
        }
      });
    ...
    const cambiarCursa = (key) => {
      ...
      socket.on("connect", () => {
        ...
        socket.emit("dataOnTime", {cursa: key}); // Emitir los datos formateados
      });
    ...
    return (
        <div style={styles.clas_container}>
          ...
          >
            <h2 style={{ justifyContent: "flex-start" }}>ON_TIME</h2>
            <label>
              <input
                type="checkbox"
  ...
  export default Operation_OnTime;