...
const Operation_Classificacio = () => {
  ...
  if (
      ["top10Men", "top10Women", "top10MenProv", "top10WomenProv", "top10MenPoint", "top10WomenPoint"].includes(selectedClas)
    ) {
      socket.emit(bool ? "showClassificacioOn" : "showClassificacioOff", bool);
    }
  };
  ...
  const formatDataForSocket = (rows) =>
    rows.map((r, i) => ({
      ...
      dorsal: r.doss || "-",
      equip: r.club || "-",
      temps: formatTimeTour(r.tiempo) || formatTimeTour(r.tps) || "-",
      pais: r.nacio || "-",
  ...
  const emitClasData = (tabId = selectedClas) => {
    ...
    socket.emit("dataClassificacio", dataClassificacio);
    socket.emit("dataTitolClassificacio", dataTitolClassificacio);
  };
  ...
  return (
    <div style={styles.clas_container}>
      <div style={{ display: "flex", flexDirection: "row", alignItems: "center", padding: "10px" }}>
        <h2 style={{ justifyContent: "flex-start" }}>CLASSIFICACIÓ</h2>
        <label>
          <input
            type="checkbox"
      ...
      <div style={{ display: "flex", padding: "10px", justifyContent: "space-between" }}>
        {[
          { id: "top10Men", label: "TOP 10 MEN" },
          { id: "top10Women", label: "TOP 10 WOMEN" },
          { id: "top10MenProv", label: "TOP 10 MEN PROV" },
          { id: "top10WomenProv", label: "TOP 10 WOMEN PROV" },
          { id: "top10MenPoint", label: "TOP 10 MEN POINT" },
          { id: "top10WomenPoint", label: "TOP 10 WOMEN POINT" },
        ].map((tab) => (
          <div
            key={tab.id}
