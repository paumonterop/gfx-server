...
const Operation_Velocitat = () => {
  ...
  const showVeloButton = (bool) => {
    ...
    if (bool) {
      socket.emit("showVelocitatOn", true); // Emitir datos para activar la velocidad
    } else {
      socket.emit("showVelocitatOff", false); // Emitir datos para desactivar la velocidad
    }
  };
  ...
  return (
    <div style={styles.clas_container}>
      ...
      >
        <h2 style={{ justifyContent: "flex-start" }}>VELOCITAT</h2>
        <label>
          <input
            type="checkbox"
        ...
        <div>
          ...
          >
            ...
            <option value="motx12">BBX 12</option>
...
export default Operation_Velocitat;
